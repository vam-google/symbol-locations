import unittest
import os
from pybind import pybind_wrapped as regular
from pybind import pybind_wrapped_copy as regular_copy
import pybind.pybind_wrapped
import pybind.pybind_wrapped_copy
from pybind.pybind_wrapped import _EXTRA_SYMBOL as REGULAR_EXTRA_SYMBOL
from pybind.pybind_wrapped_copy import _EXTRA_SYMBOL as REGULAR_COPY_EXTRA_SYMBOL
from pybind.pybind_wrapped.sub import second_func as sub_second_func
import pybind.pybind_wrapped.sub
from pybind.pybind_wrapped.sub._sub_private import *
from pybind.pybind_wrapped.sub._sub_private import _sub_private_private_func
from pybind.sub_pybind.relative_wrapped_import_lib import call_nested_pyind_func
from pybind.sub_pybind.relative_wrapped_import_lib import sub_sub_private_func
...
class PybindWrappedTest(unittest.TestCase):
  ...
  def test_pybind_first(self):
    ...
    print("2: pybind.pybind_wrapped.second_func")
    self.assertEqual(pybind.pybind_wrapped.second_func(1), 2)
    print("3: regular.third_func")
    self.assertEqual(regular.third_func(1), 1)
    print("4: pybind.pybind_wrapped_copy.first_func")
    self.assertEqual(pybind.pybind_wrapped_copy.first_func(1), 4)
    ...
    self.assertEqual(pybind.pybind_wrapped.sub.second_func(1), 6)
    self.assertEqual(pybind.pybind_wrapped.sub._sub_private.sub_sub_func(3), 6)
