...
def filter_def_file(def_file: str, filter_file: str,
  ...
  with open(filter_file, 'r', encoding='utf-8') as filter:
    ...
    inclusion_patterns: List[Pattern] = filter_json["global"] + [
        "EXPORTS", "\\s*;"
    ]

    incl_patterns = [re.compile(p) for p in inclusion_patterns]
    exclusion_patterns: List[Pattern] = filter_json["local"]
    excl_patterns = [re.compile(p) for p in exclusion_patterns]
